...
| Song/Mixer | `daw/mixer.ts`, `daw/MixerPanel.vue` | the song's name and description (the description is ours, kept in the song file only), tempo, swing, channels and faders, board rows, echo, reverb, loop — the song's own fields — plus the engine switches (`controls/engine.ts`) and the meters; the name in the top bar is read-only |
