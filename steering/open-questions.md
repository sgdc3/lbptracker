...
## 36. One island in one archived chunk will not parse — narrowed to 3 bytes

Found 2026-09-05 while answering question 35; narrowed the same day. Of **32 `CHKb` chunks** pulled
out of the public archive — 19 from "Adventure Time: The Land Of Ooo", 3 from "My Little BiG Space",
10 from a streaming music level — **31 parse clean and one reports a single bad island**.

**It is chunk `9712345a…`, island 48 of 167.** (The message used to say "island 1": it numbered the
*problems*, not the islands, and finding island 48 by hand was the first hour of this. Fixed.)

### What is known

| | |
|---|---|
| the reader's Thing marker read | byte **65735**, value `0x6e` |
| the **true** marker (`0xaa`) | byte **65732**, followed by uid `0x00026ee6` = 159206 |
| so the error is | an over-read of **exactly 3 bytes** |
| where it starts | after byte **64979** — Thing 159577's marker read correctly there |
| the frame it is in | `THING 159577` (64979-65640), whose only part is `SWITCH` (65018-65640) |

❗ **3 bytes is the difference between an `i32` and a `u8`**, which is the shape of this bug: one
field in `readSwitch` is almost certainly the wrong width on this revision. The revision is
`v0x3f9 sub0x218 branch 0/0 cf0` — note **`cf0`**, an *uncompressed* stream, where every integer is
its full width and a wrong one costs the full difference. The corpus the part readers were built on
is mostly `cf7`.

⚠️ `readSwitch` has two data-dependent reads (`s.resource(true)` under `type === SWITCH_STICKER`,
and another under `SWITCH_MICROCHIP && includeTouching === 1`), and its own comment already records
that `type` is a **zigzag** `s32` whose value decides them. A wrong `type` is the first thing to
check: it costs bytes only on the switches that carry a plan, which is exactly the "one island in
167" shape.

### The anchor

The read log is reproducible. Open the chunk, take `plans[48]`, read its `PLNb` into a `Serializer`
over `thingData`, wrap every method to log `(name, before, after)` in a window, and run
`readThingRef` in a loop — `setTrace` from `thing.ts` interleaves the Thing and part spans. Reading
back from 65732 the tail is unambiguous:

```
65644..65708  matrix        parentBoneOffset
65708..65709  u8            playerMode
65709..65710  u8            layerRange      <- CONTROLINATOR ends; should be 65707
65710..65731  5x f32, bool  the enclosing structure
65731..65735  i32           the next Thing's reference id
65735         THING         marker 0x6e, three bytes late
```

⚠️ The probe needs `readIsland` exported from `level.ts`; it is not, deliberately, so a one-line
change makes it visible again.
