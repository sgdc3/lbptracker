...
## 30. The note word's bits 28..29 pick one of four (level, pan, echo, reverb, instrument) tuples

Pulled out of question 29's loose end. Two things are now measured and the third is a hypothesis.

### `[rbp-0x78]` is the mixer channel's volume

`sub_0x280`, the caller that allocates a voice, builds it before the loop:

```
0x03bf  eax = ecx & ~7                ; the signed mod-8 idiom
0x03c2  ecx = ecx mod 8               ; the channel
0x03c7  rax = ecx * 3                 ; three dwords -- 12 bytes per channel record
0x03d4  rcx = rdi + rax*4 + 0x40
0x03d9  [rbp - 0x78] = rcx            ; -> that channel's volume
```

Same `mod 8` and the same 12-byte record the mixer-channel note in `project.ts` already carries. So
the test that decides whether a note gets a voice record at all is

```
0x04cb  vmulss xmm0, xmm0, [rbx + rax*4 + 0x420]   ; channelVolume x blockGain
0x04d4  vucomiss xmm0, 0
0x04dc  jbe 0x410                                  ; not positive: no record
```

### The table at `+0x420` is a clip's last 80 bytes, five columns wide

`rbx` walks an array of **0x470-byte** records (`0x02db`, `0x0399`). The same record holds the note
count at `+0x1c` and the note words at `+0x20` — `(0x420 - 0x20) / 4 = 256` records — so **the
record is a clip**, and `+0x420`…`+0x470` is `4 x 20` bytes at the very end of it. All five columns
are used, indexed by `bits28..29 * 5` dwords:

| column | where | used as |
|---|---|---|
| `+0x420` | `0x04cb`, `0x3b09` | a **gain**, multiplied into the volume test |
| `+0x424` | `0x3b20` | a float |
| `+0x428` | `0x3c8a` | a float |
| `+0x42c` | `0x3cf8` | a float |
| `+0x430` | `0x0510`, `0x09bf`, `0x3b17` | an int whose **low byte indexes instrument records** (`x 0x5f0`) |

Five columns against `PInstrument`'s five output fields — `Level`, `Pan`, `EchoSend`, `ReverbSend`
and `Instrument` — is not a coincidence worth ignoring.

### The corpus uses the field, heavily

1,739,058 note records over 74,864 clips, ten levels plus the checkout:
...
**70,302 of 74,864 clips (94%) use more than one**, and the value is **sticky**: 90.4% of records
repeat the previous record's, against 51% for the record index's parity and 49.7% for the step's. It
runs in long blocks. It is not noise and it is not positional.

**And this project ignores the field entirely.**

### ⚠️ The eboot writes entry 0 and only entry 0 — which contradicts the above

`v0x1c4420` is the writer. It indexes the clip array exactly as the plugin reads it —
`imul esi, r12d, 0x470; add rsi, [r14 + 0x1ad0]`, with `+0x1ad8` as the other half of the double
buffer — and then:

```
v0x1c44c9  mov dword [rax + 0x430], r13d     ; the instrument index
v0x1c450e  vmulss xmm0, xmm0, [rax + 0x420]  ; and the gain, scaled in place by a bool
v0x1c4516  vmovss [rax + 0x420], xmm0
```

**No `* 20` and no index.** Only entry 0 is ever written from here.

❗ **So the two readings cannot both be right.** If the plugin indexed `+0x420 + 20k` by the note's
bits 28..29 on the same record the eboot fills, then the 52% of notes selecting 1, 2 or 3 would
multiply by an unwritten gain, fail `0x04dc`'s `jbe`, and never sound. Half of every song would be
missing, and it is not.

**Where the misreading probably is**: the base at `0x04cb`. `sub_0x280` carries two cursors over the
same placement index — `r14 = i * 0x470` and `r8 = i * 0x10` — and two bases, `[rdi + 0xb0]` and
`[rdi + 0xa8]`, taken on either side of a double-buffer flag. The `0x470` branch reads
`[rbx + r9*4 + 0x20]` for the note word and so is a clip; the `0x10` branch reads the *same*
expression, which a 16-byte record cannot hold. One of the two is not what it looks like, and until
that is settled the `+0x420 + 20k` reading is not safe to build on.

### The hypothesis, and why it is not alarming yet

If bits 28..29 chose the **instrument**, half the notes of nearly every clip would play a different
one, and every render this project has checked against a recording would be obviously wrong. They
are not. The reconciliation on the table: `PInstrument` carries **one** `Instrument`, `Level`, `Pan`,
`EchoSend` and `ReverbSend` — there is nowhere in the serialised Thing for a second tuple to come
from — so the eboot most likely fills all four identically and the select is **inert in practice**,
exactly as bit 30's resting value is.

⚠️ That is a hypothesis, not a measurement, and the 94% makes it worth settling.

**The anchor**: the eboot's DSP-record builder, `v0x2a1144` (⚠️ **not** `v0x2a1190`, which is
mid-function — see `eboot-re.md`). Disassemble it and watch what it writes into `+0x420`…`+0x470`
of a clip. If it writes one tuple four times, this closes as inert and the note says so. If it
writes four different ones, then a clip can carry four instruments, this project's one-instrument
`Track` is wrong for 94% of clips, and that is a much bigger finding than the one that started it.