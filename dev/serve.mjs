 ...
 *
 * ❗ **It makes no outbound requests, and that is a decision.** For one commit
 * it proxied a level search to zaprit.fish, which sends no CORS headers; the
 * feature worked and was removed, because a page that only works on a dev
 * machine is not a page this project wants and a file server should not also be
 * a proxy. `dev/archive-panel.ts` opens an archived level by its hash instead,
 * straight from archive.org, with nothing in between.
 */
