...
const props = defineProps<{ family: string; colour: string; size?: number }>();
...
const paint = () => {
  ...
  if (!el) return;
  el.textContent = '';
  el.append(glyphCanvas({ family: props.family, colour: props.colour }, props.size ?? 18));
};
onMounted(paint);
watch(() => [props.family, props.colour, props.size], paint);
</script>
